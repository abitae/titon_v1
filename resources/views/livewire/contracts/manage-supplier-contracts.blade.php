<div class="space-y-6">
    ...
    <x-platform.filter-bar>
        <div>
            ...
            <input wire:model.live.debounce.300ms="search" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" placeholder="Numero o tipo de contrato" />
        </div>
        <div>
    ...
    <x-platform.compact-table :headers="['Contrato', 'Obra', 'Proveedor', 'Monto', 'Estado', 'Acciones']">
        @forelse ($contracts as $contract)
            <tr class="text-sm text-slate-700 dark:text-slate-200" wire:key="contract-row-{{ $contract->id }}">
                <td class="px-6 py-4 font-medium text-slate-950 dark:text-white">{{ $contract->contract_number }}</td>
                <td class="px-6 py-4">{{ $contract->project?->name ?? 'Sin obra' }}</td>
                <td class="px-6 py-4">{{ $contract->supplier?->business_name ?? 'Sin proveedor' }}</td>
                <td class="px-6 py-4">{{ $contract->currency }} {{ number_format((float) $contract->total_amount, 2) }}</td>
                <td class="px-6 py-4"><x-platform.status-badge :value="$contract->status" /></td>
                <td class="px-6 py-4">
                    <div class="flex flex-wrap gap-2 justify-end">
                        ...
                        <a href="{{ route('contracts.pdf', $contract) }}" class="rounded-lg px-2 py-1 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:text-slate-200">PDF</a>
                    </div>
                </td>
            </tr>
        @empty
                    ...
                    <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-slate-950/30">
                        <div class="grid gap-4 md:grid-cols-2">
                            <div>
                                ...
                                <input wire:model="selectedContract.contract_number" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Tipo</label>
                                <input wire:model="selectedContract.contract_type" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Inicio</label>
                                <input wire:model="selectedContract.start_date" type="date" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Fin</label>
                                <input wire:model="selectedContract.end_date" type="date" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Monto total</label>
                                <input wire:model="selectedContract.total_amount" type="number" min="0" step="0.01" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Estado</label>
                                <select wire:model="selectedContract.status" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white">
                                    @foreach ($statusOptions as $statusOption)
                                        <option value="{{ $statusOption->value() }}">{{ $statusOption->label() }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea wire:model="selectedContract.payment_conditions" rows="2" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white"></textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Penalidades</label>
                                <textarea wire:model="selectedContract.penalties" rows="2" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white"></textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Garantias</label>
                                <textarea wire:model="selectedContract.guarantees" rows="2" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white"></textarea>
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Observacion</label>
                                <textarea wire:model="selectedContract.observation" rows="3" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white"></textarea>
                            </div>
                        </div>
                    ...
                    <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                        ...
                        <div class="mt-4 space-y-2">
                            @forelse ($selectedContract->getMedia('signed_contract') as $media)
                                <a href="{{ $media->getUrl() }}" target="_blank" class="block rounded-2xl border border-slate-200 px-4 py-3 text-sm font-medium text-cyan-700 dark:border-slate-800 dark:text-cyan-300">{{ $media->file_name }}</a>
                            @empty
                                <p class="text-sm text-slate-500 dark:text-slate-400">No hay contrato firmado adjunto.</p>
                            @endforelse
                        </div>
                        <input wire:model="signed_contract_files" type="file" multiple class="mt-4 block w-full rounded-xl border border-dashed border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
