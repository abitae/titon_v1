<div class="space-y-6">
    ...
    <form wire:submit="send" class="rounded-3xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
        <div class="grid gap-4 md:grid-cols-2">
            <div class="md:col-span-2">
                ...
                <select wire:model="supplier_ids" multiple class="mt-2 block min-h-32 w-full rounded-xl border border-slate-300 bg-white px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white">
            ...
            <div>
                ...
                <input type="date" wire:model="response_deadline" class="mt-2 block w-full rounded-xl border border-slate-300 px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
            </div>
            <div class="md:col-span-2">
                <label class="text-sm font-medium">Mensaje</label>
                <textarea wire:model="message" rows="3" class="mt-2 block w-full rounded-xl border border-slate-300 px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white"></textarea>
            </div>
        </div>
    ...
    <div class="rounded-3xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
        ...
        <x-platform.compact-table class="mt-4" :headers="['Proveedor', 'Estado', 'Enviado', 'Límite']">
            @forelse ($invitations as $invitation)
                <tr wire:key="invitation-{{ $invitation->id }}">
                    <td>{{ $invitation->supplier?->business_name }}</td>
                    <td>{{ $invitation->status }}</td>
                    <td class="whitespace-nowrap">{{ $invitation->sent_at?->format('d/m/Y H:i') }}</td>
                    <td class="whitespace-nowrap">{{ $invitation->response_deadline?->format('d/m/Y') }}</td>
                </tr>
            @empty
                <tr><td colspan="4" class="!py-5 text-center text-[11px] text-slate-500">Sin invitaciones.</td></tr>
            @endforelse
