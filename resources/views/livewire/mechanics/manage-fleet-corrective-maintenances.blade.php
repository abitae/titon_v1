    ...
    <div class="overflow-x-auto rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <table class="min-w-full text-sm">
            ...
            <tr>
                ...
                <th class="px-4 py-3">Fecha falla</th>
            ...
            @forelse ($rows as $row)
                <tr wire:key="corr-{{ $row->id }}">
                    ...
                    <td class="px-4 py-3">{{ optional($row->failure_at)->format('d/m/Y H:i') }}</td>
                    <td class="px-4 py-3">{{ $row->supplier_workshop ?? '—' }}</td>
                    <td class="px-4 py-3">{{ $row->real_cost ?? '—' }}</td>
                    <td class="px-4 py-3 text-end">
                        @can('mantenimientos.crear')
                            <button type="button" wire:click="openEdit({{ $row->id }})" class="text-sm text-cyan-700">Editar</button>
                        @endcan
        ...
        <div class="fixed inset-0 z-50 flex justify-center overflow-y-auto bg-slate-950/60 px-4 py-8 backdrop-blur-sm">
            <div class="w-full max-w-2xl rounded-3xl border bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-900">
                ...
                <div class="mt-4 grid gap-3">
                    <select wire:model="fleet_equipment_id" class="rounded-xl border px-3 py-2 dark:border-slate-700 dark:bg-slate-950">
                        @foreach ($equipments as $equipment)
                            <option value="{{ $equipment->id }}">{{ $equipment->internal_code }} · {{ $equipment->name }}</option>
                        @endforeach
                    </select>
                    <input type="datetime-local" wire:model="failure_at" class="rounded-xl border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" />
                    <textarea wire:model="failure_description" rows="2" class="rounded-xl border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" placeholder="Descripcion falla"></textarea>
                    <textarea wire:model="diagnosis" rows="2" class="rounded-xl border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" placeholder="Diagnostico"></textarea>
                    <input wire:model="supplier_workshop" class="rounded-xl border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" placeholder="Proveedor / taller" />
                    <div class="grid grid-cols-2 gap-2">
                        <input wire:model="estimated_cost" class="rounded-xl border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" placeholder="Costo estim." />
                        <input wire:model="real_cost" class="rounded-xl border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" placeholder="Costo real" />
                    </div>
                    ...
                    <textarea wire:model="observations" rows="2" class="rounded-xl border px-3 py-2 dark:border-slate-700 dark:bg-slate-950"></textarea>
