        ...
        <div class="space-y-4 p-4 sm:p-6" role="tabpanel">
            ...
            <x-platform.filter-bar class="xl:grid-cols-2">
                <div>
                    ...
                    <input wire:model.live.debounce.300ms="search" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" placeholder="Nombre o código" />
                </div>
                <div>
            ...
            <x-platform.compact-table :headers="['Nombre', 'Código', 'Estado', 'Orden', 'Acciones']">
                @forelse ($items as $item)
                    <tr class="text-xs text-slate-700 dark:text-slate-200" wire:key="catalog-row-{{ $item->id }}">
                        ...
                        <td class="px-2.5 py-1.5">{{ $item->code ?: 'Sin código' }}</td>
                        <td class="px-2.5 py-1.5">
                            <x-platform.status-badge :value="$item->is_active ? 'active' : 'inactive'" />
                        </td>
                        <td class="px-2.5 py-1.5">{{ $item->sort_order }}</td>
                        <td class="px-2.5 py-1.5">
