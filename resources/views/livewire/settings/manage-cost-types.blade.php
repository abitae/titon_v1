<div class="space-y-6">
    ...
    <x-platform.filter-bar class="xl:grid-cols-2">
        <div>
            ...
            <input wire:model.live.debounce.300ms="search" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" placeholder="Nombre o codigo" />
        </div>
        <div>
            <label class="block text-xs font-medium uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">Estado</label>
            <select wire:model.live="activeFilter" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white">
    ...
    <x-platform.compact-table :headers="['Nombre', 'Codigo', 'Estado', 'Orden', 'Acciones']">
        @forelse ($costTypes as $costType)
            <tr class="text-xs text-slate-700 dark:text-slate-200" wire:key="cost-type-{{ $costType->id }}">
                ...
                <td class="px-2.5 py-1.5">{{ $costType->code ?: 'Sin codigo' }}</td>
                <td class="px-2.5 py-1.5">
                    <x-platform.status-badge :value="$costType->is_active ? 'active' : 'inactive'" />
                </td>
                <td class="px-2.5 py-1.5">{{ $costType->sort_order }}</td>
                <td class="px-2.5 py-1.5">
    ...
    <x-platform.modal :show="$showFormModal" max-width="max-w-2xl">
        ...
        <div class="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <div>
                ...
                <input wire:model="code" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
            ...
            <div class="md:col-span-2">
                ...
                <textarea wire:model="description" rows="3" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white"></textarea>
